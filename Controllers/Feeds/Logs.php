...
class Logs extends Generic
{
	...
	protected function fetchLogs()
	{
		try
		{
			$files = $this->sourceControlClient->log($this->filePath,$this->resultSize, $this->project);
		}
		catch(\Viacom\Crabapple\Exceptions\Exception $e)
		{
			$this->logException('Failed to fetch logs for path: '.$this->filePath.' for projectId: '.$this->projectId,$e);
			$this->requestFailed('Failed to fetch logs for path: '.$this->filePath);
			return;
		}
